...
def crypt(message='Hello World!', n=3, cipher_matrix=np.array([0])):
        ...
        asciidict=OrderedDict(zip(a,range(0,97)))
        numbdict=OrderedDict(zip(range(0,97),a))
        msg=message
        ...
        if np.array_equal(cipher_matrix, [0]):
                cipher_matrix=np.random.randint(0,97,(n,n))
        ...
        if err == 0:
                ...
                return
        b=b[0].item(0)+1
        key_matrix=np.mod(b*P,97).astype(int)
